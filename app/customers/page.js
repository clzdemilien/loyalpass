    ...
    <div style={{ minHeight: '100vh', background: '#0A0A0F', color: '#F0F0F5', fontFamily: "'Inter', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-scan { background:linear-gradient(135deg,#8B5CF6,#6D28D9); border:none; border-radius:16px; padding:16px; color:#fff; font-size:16px; font-weight:700; cursor:pointer; font-family:'Space Grotesk',sans-serif; width:100%; transition:all 0.2s; -webkit-tap-highlight-color:transparent; letter-spacing:-0.3px; }
        .btn-scan:hover { opacity:0.9; transform:translateY(-2px); box-shadow:0 12px 40px rgba(139,92,246,0.4); }
        .btn-scan:active { transform:scale(0.97); }
        .btn-copy { border:none; border-radius:12px; padding:12px 20px; font-size:14px; font-weight:600; cursor:pointer; font-family:inherit; transition:all 0.2s; -webkit-tap-highlight-color:transparent; }
        .tx-row { display:flex; align-items:center; justify-content:space-between; padding:12px 16px; border-bottom:1px solid rgba(255,255,255,0.04); transition:background 0.15s; }
      ...
      <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 0 }}>
        ...
        <div style={{ position: 'absolute', bottom: '-15%', left: '-5%', width: '400px', height: '400px', background: 'radial-gradient(circle, rgba(245,158,11,0.06) 0%, transparent 70%)', borderRadius: '50%' }} />
      ...
      <nav style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', height: '60px', display: 'flex', alignItems: 'center', background: 'rgba(10,10,15,0.85)', backdropFilter: 'blur(16px)', position: 'sticky', top: 0, zIndex: 100 }}>
        <div className="nav-pad" style={{ width: '100%', maxWidth: '680px', margin: '0 auto', padding: '0 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => router.push(`/scanner?code=${customer?.qr_code}`)} style={{ background: 'rgba(139,92,246,0.12)', border: '1px solid rgba(139,92,246,0.3)', borderRadius: '10px', padding: '7px 14px', color: '#A78BFA', fontSize: '13px', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit' }}>
            📷 Scanner
          </button>
        </div>
      </nav>
      ...
      <div className="main-pad" style={{ maxWidth: '680px', margin: '0 auto', padding: '1.75rem 2rem', position: 'relative', zIndex: 1 }}>
        ...
        <div className="fade-up" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '24px', padding: '1.75rem', marginBottom: '1rem' }}>
          ...
          <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem', marginBottom: loyaltyCard ? '1.25rem' : '0' }}>
            {[
              ...
              { label: 'Récomp.', value: '—', color: '#10B981', icon: '🎁' },
            ].map(s => (
              <div key={s.label} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', padding: '1rem', textAlign: 'center' }}>
        ...
        <div className="fade-up" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '24px', padding: '1.5rem', marginBottom: '1rem', textAlign: 'center', animationDelay: '0.2s' }}>
          ...
          <div style={{ display: 'inline-block', padding: '1.25rem', background: '#fff', borderRadius: '20px', marginBottom: '1.25rem', boxShadow: '0 0 40px rgba(139,92,246,0.2)' }}>
            ...
            {transactions.map((tx, i) => (
              <div key={tx.id} className="tx-row">
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div>
                    <p style={{ fontSize: '13px', fontWeight: '500' }}>+{tx.points_added} point{tx.points_added > 1 ? 's' : ''}</p>
                    <p style={{ fontSize: '11px', color: '#6B7280' }}>{formatDate(tx.created_at)}</p>
                  </div>
                </div>
                <div style={{ fontSize: '13px', fontWeight: '600', color: '#8B5CF6' }}>+{tx.points_added}</div>
              </div>
            ))}
          </div>
        )}
