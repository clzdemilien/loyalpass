    ...
    <div style={{ minHeight: '100vh', background: '#0A0A0F', color: '#F0F0F5', fontFamily: "'Inter', sans-serif", overflowX: 'hidden' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .feature-card { background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.07); border-radius:24px; padding:2rem; transition:all 0.4s; }
      ...
      <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '7rem 2rem 5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center', position: 'relative', zIndex: 1 }}>
        <div>
          ...
          <h1 className="fade-up" style={{ fontSize: 'clamp(2.5rem, 5vw, 3.8rem)', fontWeight: '800', lineHeight: 1.05, letterSpacing: '-0.04em', marginBottom: '1.5rem', fontFamily: 'Space Grotesk, sans-serif', animationDelay: '0.1s' }}>
            Fidélisez vos clients <span className="shine">automatiquement</span>
          </h1>
          ...
          <div className="fade-up" style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', animationDelay: '0.3s' }}>
            <button className="btn-cta" onClick={() => router.push('/auth/register')}>Créer ma carte gratuite ✦</button>
          ...
          <div className="fade-up" style={{ display: 'flex', gap: '2rem', animationDelay: '0.4s' }}>
            {[
              ...
              <div key={s.label} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '20px', fontWeight: '800', color: '#8B5CF6', fontFamily: 'Space Grotesk, sans-serif' }}>{s.value}</div>
          ...
          <div className="float glow" style={{ width: '300px', zIndex: 2 }}>
            <div className="wallet-card" style={{ background: 'linear-gradient(135deg, #8B5CF6, #4C1D95)', boxShadow: '0 24px 80px rgba(139,92,246,0.5)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
                <div>
                  <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', marginBottom: '4px' }}>LoyalPass</div>
                  <div style={{ fontSize: '16px', fontWeight: '700', color: '#fff' }}>Le Bistrot du Coin</div>
                </div>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', marginBottom: '6px' }}>Points fidélité</div>
                <div style={{ fontSize: '36px', fontWeight: '800', color: '#fff', lineHeight: 1 }}>7 <span style={{ fontSize: '16px', opacity: 0.5 }}>/ 10</span></div>
              </div>
              <div style={{ display: 'flex', gap: '6px', marginBottom: '1.25rem' }}>
                {[...Array(10)].map((_, i) => (
                  <div key={i} style={{ width: '22px', height: '22px', borderRadius: '50%', background: i < 7 ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.15)', border: i < 7 ? 'none' : '1.5px solid rgba(255,255,255,0.2)' }} />
                ))}
              </div>
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)', padding: '1.5rem 3rem', display: 'flex', justifyContent: 'center', gap: '4rem', background: 'rgba(255,255,255,0.02)', position: 'relative', zIndex: 1, flexWrap: 'wrap' }}>
        {[
      ...
      <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '6rem 2rem', position: 'relative', zIndex: 1 }}>
        <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
          ...
          <h2 style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: '800', letterSpacing: '-0.04em', fontFamily: 'Space Grotesk, sans-serif', marginBottom: '1rem' }}>
            Tout ce qu'il faut, <span className="shine">rien de superflu</span>
          </h2>
          <p style={{ color: '#6B7280', fontSize: '16px', maxWidth: '500px', margin: '0 auto', lineHeight: 1.7, fontWeight: '300' }}>Conçu pour les commerçants qui veulent des résultats sans la complexité.</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
          {[
            { icon: '📷', title: 'Scanner QR instantané', desc: 'Scannez en 2 secondes depuis votre téléphone. Les points s\'ajoutent automatiquement.', color: '#8B5CF6' },
            { icon: '💳', title: 'Apple & Google Wallet', desc: 'Vos clients ajoutent leur carte en un clic. Mise à jour en temps réel avec leurs points.', color: '#06B6D4' },
            { icon: '🎁', title: 'Récompenses automatiques', desc: 'Quand un client atteint son objectif, la récompense se déclenche automatiquement.', color: '#10B981' },
            { icon: '🤖', title: 'Relances automatiques', desc: 'Un client inactif depuis 30 jours reçoit automatiquement une offre pour revenir.', color: '#F59E0B' },
            { icon: '📊', title: 'Analytics précis', desc: 'Taux de retour, meilleurs clients, scans par mois — toutes vos données en un coup d\'œil.', color: '#EC4899' },
            { icon: '⚡', title: 'Setup en 5 minutes', desc: 'Créez votre carte, définissez vos récompenses, publiez. Résultats immédiats.', color: '#A78BFA' },
          ].map(f => (
            <div key={f.title} className="feature-card">
              ...
              <h3 style={{ fontSize: '16px', fontWeight: '700', marginBottom: '0.75rem', fontFamily: 'Space Grotesk, sans-serif' }}>{f.title}</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
            {[
              ...
              <div key={step.num} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '20px', padding: '1.5rem', position: 'relative' }}>
                <div style={{ fontSize: '11px', color: '#8B5CF6', fontWeight: '700', marginBottom: '0.75rem' }}>{step.num}</div>
      ...
      <section style={{ maxWidth: '800px', margin: '0 auto', padding: '6rem 2rem', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
          {[
            { name: 'Starter', price: '29,90€', period: '/mois', popular: false, color: '#8B5CF6', features: ['1 carte fidélité', '500 clients', 'Scanner QR', 'Apple & Google Wallet', 'Notifications push', 'Automatisations de base'] },
            { name: 'Business', price: '79,90€', period: '/mois', popular: true, color: '#06B6D4', features: ['Cartes illimitées', 'Clients illimités', 'Scanner QR', 'Apple & Google Wallet', 'Automatisations complètes', 'Analytics avancés', 'Multi-employés', 'IA & campagnes auto'] },
          ].map(plan => (
      ...
      <section style={{ padding: '5rem 2rem', position: 'relative', zIndex: 1, textAlign: 'center' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', background: 'rgba(139,92,246,0.06)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: '32px', padding: '4rem 3rem' }}>
          ...
          <p style={{ color: '#6B7280', fontSize: '15px', lineHeight: 1.7, marginBottom: '2rem', fontWeight: '300' }}>
            Rejoignez 2 400 commerçants qui ont modernisé leur fidélité. Configuration en 5 minutes.
          </p>
      ...
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.05)', padding: '2rem 3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', zIndex: 1, flexWrap: 'wrap', gap: '1rem' }}>
